...
└── src/
    ...
    ├── api/
    │   ├── auth.js             # 로그인 / 회원가입- `src/api/`: API 호출 로직 분리
    - `auth.js`: **중앙 axios 인스턴스 및 BASE_URL 관리** (환경 변수 참조)
    - `calendar.js`, `friends.js`, `mypage.js`, `post.js`
    │   ├── friends.js          # 친구 요청 / 목록 API
    │   └── post.js             # 게시물 관련 API
    ├── stores/
    │   └── counter.js          # Pinia 스토어
    ├── components/
    │   ├── EventModal.vue      # 이벤트 생성/상세 모달 (네이버 지도 포함)
    │   ├── FriendManagerModal.vue  # 친구 관리 모달
    │   ├── NaverMapSelector.vue # [NEW] 장소 선택을 위한 지도 컴포넌트
    │   └── ProfileCard.vue     # 프로필 카드 컴포넌트
    └── views/
        ├── CalendarView.vue    # 메인 캘린더 화면 (월별/주별 뷰)
        ├── LoginView.vue       # 로그인 / 회원가입 화면
        ├── MyPageView.vue      # [NEW] 회원 정보 및 도착 통계 화면
        └── LiveMeetingView.vue # [NEW] 실시간 약속 대시보드 (위치 공유/채팅)
```
...
- 약속 생성 시 **네이버 지도(oapi)**에서 장소를 선택하고 친구를 초대하는 로직 포함.

### `/live/:id` — 실시간 대시보드 (LiveMeetingView.vue) [NEW]
- 네이버 지도를 통해 참여자들의 **실시간 좌표**를 동기화.
- 약속 장소 반경 이내 진입 시 **자동 체크인** 애니메이션 및 알림.
- 휘발성 **임시 그룹 채팅** 공간 제공.

### `/mypage` — 내 정보 (MyPageView.vue) [NEW]
- 프로필 이미지 업로드 및 개인 정보 수정.
- **약속 준수율(Manner Score)** 통계 대시보드 제공.
...
### `NaverMapSelector.vue` — 지도 선택기 [NEW]
- **네이버 클라우드 통합 콘솔(oapi)** 스크립트를 로드하여 인터랙티브 맵 구현.
- **장소명 검색**: 백엔드 `/places/search` API를 호출하여 장소명 키워드로 검색하고, 결과를 드롭다운 목록으로 표시해 선택.
- **지도 클릭 지정**: 지도 클릭 시 역지오코딩(Reverse Geocode)으로 주소를 자동 추출하여 부모 컴포넌트로 전달.
- **Robustness**: 모달 애니메이션 및 컨테이너 크기 변화에 대응하기 위해 `nextTick` 및 `resize` 이벤트 핸들링 로직 포함.

### `App.vue` — 전역 레이아웃
- **상단 네비게이션 바**: 로고(`logo_buddy.png`), 다크 모드 토글 (필터 없음), 친구 관리, 로그인/로그아웃 버튼
- **사이드바**: 일정 필터 (내 일정 / 친구 일정)
- 라우트 변경 시 로그인 상태 자동 갱신 및 로고 CSS 정리 완료
...
> **알려진 이슈 (해결됨)**: vue-cal 내부 메뉴 버튼(월간/주간/일간 탭)이 라이트 모드에서 흰색 텍스트로 표시되던 문제를 `:deep(.vuecal__menu li)` 스타일 추가로 해결. (2026-05-12)
